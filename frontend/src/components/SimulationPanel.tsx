...
interface Props {
  simulation?: {
    ...
    spread_history: { step: number; activation_pct: number; avg_sentiment: number }[]
    group_breakdown: Record<string, { activated_pct: number; avg_sentiment: number; total: number }>
  }
}
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="bg-bg-card rounded-lg border border-border p-3 text-center">
          <div className="text-lg font-bold text-white mono">
            {(simulation.total_agents / 1000).toFixed(0)}K
          </div>
          <div className="text-[9px] text-zinc-600">{t('stats.agents')}</div>
        </div>
      </div>
